...
namespace ServiceRequestInformationSystem
{
    public partial class ucAccounts : UserControl
    {
        ...
        private void dataGridView_UserList_DoubleClick(object sender, EventArgs e)
        {
            ...
            tb_LastName.Text = dataGridView_UserList.SelectedRows[0].Cells[4].Value.ToString(); 
            cb_AccessLevel.SelectedItem = dataGridView_UserList.SelectedRows[0].Cells[5].Value.ToString();
            checkBox_Status.Checked = Convert.ToBoolean(dataGridView_UserList.SelectedRows[0].Cells[6].Value.ToString());
        }
    }
}
